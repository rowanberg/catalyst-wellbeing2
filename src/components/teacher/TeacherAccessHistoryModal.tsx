...
interface TeacherAccessHistoryModalProps {
    open: boolean
    onOpenChange: (open: boolean) => void
}

export function TeacherAccessHistoryModal({ open, onOpenChange }: TeacherAccessHistoryModalProps) {
    ...
    const fetchLogs = async () => {
        try {
            setLoading(true)
            setError(null)
            const res = await fetch('/api/teacher/access-logs', {
                credentials: 'include'
            })
            if (!res.ok) {
