...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      if (appError.message.includes('confirm your email') || appError.message.includes('Email not confirmed')) {
  ...
  const handleVerifyOTP = async (otpCode: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Attempt login again with the stored password (if available) or ask user to login
        // Since we don't store the password in state for security, we'll ask them to login again
        // or we could try to use the form data if we had access to it here.
        // Better UX: Just hide OTP and let them click Sign In again, or auto-submit if we kept the password.
        // For now, let's just close OTP and fill the email.
        setShowOTP(false)
        setValue('email', otpEmail)

        // Ideally, we would auto-login here, but we need the password.
        // Let's prompt them to sign in again.
        addToast({
          type: 'success',
                  ...
                  <div className="space-y-6">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        We've sent a code to <span className="font-medium text-blue-600 dark:text-blue-400">{otpEmail}</span>
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <button
                        ...
                        className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 font-medium disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-2 px-4 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-200 font-medium rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
                      >
                        Back to Login
                      </button>
                    </div>
