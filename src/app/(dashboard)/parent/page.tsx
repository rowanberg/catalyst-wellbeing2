...
import { useState, useEffect, useCallback, Suspense, lazy, useMemo, memo } from 'react'
...
function ParentDashboardContent() {
  ...
  // Memoize children and details
  const children = useMemo(() => parentData?.children || [], [parentData?.children])
  const selectedChildDetails = useMemo(() => {
  ...
  const handleChildChange = useCallback((childId: string) => {
    ...
    if (child) {
      // Track child switch
      track('parent_child_switch', {
        from_child: selectedChild,
        to_child: childId,
        tab: activeTab
      })
    }
    checkNotifications()
  }, [updateChild, children, selectedChild, activeTab, track, checkNotifications])
  ...
  // Check notifications when selected child changes
  useEffect(() => {
    if (selectedChild) {
      checkNotifications()
    }
  }, [selectedChild, checkNotifications])
