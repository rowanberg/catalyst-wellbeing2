...
export default function AIAssistantPage() {
  ...
  const [aiService, setAIService] = useState<HuggingFaceService | null>(null)
...
I can analyze this data to provide insights, answer questions about school performance, and help with decision-making. What would you like to explore today?`,
  ...
  // Listen for changes in AI configuration
  useEffect(() => {
  ...
  const loadAIConfiguration = () => {
    const savedConfig = localStorage.getItem('catalyst-ai-settings')
    if (savedConfig) {
      const config = JSON.parse(savedConfig)
      setAIConfig(config)
      
      // Always create AI service if we have any config, even without API key
      // Our intelligent fallback system doesn't require external APIs
      const service = new HuggingFaceService({
        apiKey: config.huggingFaceApiKey || 'fallback',
        model: config.model || 'google/flan-t5-large'
      })
      setAIService(service)
    } else {
      // Even without saved config, create a default service for intelligent responses
      const defaultService = new HuggingFaceService({
        apiKey: 'fallback',
        model: 'google/flan-t5-large'
      })
      setAIService(defaultService)
      setAIConfig(null)
    }
  }
  ...
  const initializeAI = async () => {
    try {
      ...
      // Generate initial insights if AI is configured
      const savedConfig = localStorage.getItem('catalyst-ai-settings')
      if (savedConfig) {
        const config = JSON.parse(savedConfig)
    ...
    const fallbackInsights: AIInsight[] = [
      {
        ...
        description: 'Set up your Hugging Face API key to enable AI-powered insights and recommendations.',
        priority: 'high',
        timestamp: 'Now',
        actionable: true,
        impact: 'moderate',
        confidence: 95,
        affectedStudents: 0,
        recommendedAction: 'Go to Settings tab and add your API key'
      }
    ]
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (geminiResponse.ok) {
        ...
        setChatMessages(prev => {
          ...
          return [...filtered, {
            ...
            type: 'assistant',
            content: errorData.error || 'Failed to get response from Gemini API',
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }]
        })
        ...
        if (aiService && schoolContext) {
          ...
          setChatMessages(prev => {
            const filtered = prev.filter(msg => !msg.isLoading)
            return [...filtered, {
              id: `fallback-${timestamp}-${Math.random().toString(36).substr(2, 9)}`,
              type: 'assistant',
              content: response,
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }]
          ...
          setConversationHistory(prev => [...prev, { role: 'assistant' as const, content: response }])
        } else if (schoolContext) {
          // Create temporary service for intelligent responses
          const tempService = new HuggingFaceService({
            apiKey: 'fallback',
            model: 'google/flan-t5-large'
          })
          
          const response = await tempService.generateResponse(
            userMessage.content,
            schoolContext,
            conversationHistory
          )
          ...
          setChatMessages(prev => {
            const filtered = prev.filter(msg => !msg.isLoading)
            return [...filtered, {
              id: `nocontext-${timestamp}-${Math.random().toString(36).substr(2, 9)}`,
              type: 'assistant',
              content: 'Loading school data... Please wait a moment and try again.',
              timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }]
          })
        }
      ...
      setChatMessages(prev => {
        const filtered = prev.filter(msg => !msg.isLoading)
        return [...filtered, {
          id: `catch-${timestamp}-${Math.random().toString(36).substr(2, 9)}`,
          type: 'assistant',
  ...
  const generateAIInsights = async (context: SchoolContext) => {
    ...
    try {
      // Generate AI insights based on school context data
      const mockInsights: AIInsight[] = [
        {
          id: '1',
          type: 'wellbeing',
          title: 'Rising Anxiety Levels Detected',
          description: `Analysis shows a 15% increase in anxiety-related help requests over the past week. ${context.wellbeingMetrics.helpRequests} students have requested support.`,
          priority: 'high',
          timestamp: 'Just now',
          actionable: true,
          impact: 'moderate',
          confidence: 87,
          affectedStudents: Math.floor(context.totalStudents * 0.15),
          recommendedAction: 'Consider implementing mindfulness sessions and reviewing current stressors'
        },
        {
          id: '2',
          type: 'academic',
          title: 'Engagement Drop in Mathematics',
          description: `Academic performance data indicates a 12% decrease in math engagement scores. ${context.academicMetrics.strugglingStudents} students need additional support.`,
          priority: 'medium',
          timestamp: '2 hours ago',
          actionable: true,
          impact: 'moderate',
          confidence: 82,
          affectedStudents: Math.floor(context.totalStudents * 0.3),
          recommendedAction: 'Review math curriculum and consider gamification strategies'
        },
        {
          id: '3',
          type: 'behavioral',
          title: 'Positive Peer Interactions Increasing',
          description: 'Behavioral data shows a 20% increase in positive peer interactions and collaborative activities.',
          priority: 'low',
          timestamp: '1 day ago',
          actionable: false,
          impact: 'low',
          confidence: 91,
          affectedStudents: Math.floor(context.totalStudents * 0.8),
          recommendedAction: 'Continue current social-emotional learning initiatives'
        }
      ]
