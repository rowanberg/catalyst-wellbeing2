...
const StudentDashboardContent = () => {
  ...
  const fetchDashboardData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      // Use OfflineAPI for PWA offline support
      try {
        // Fetch dashboard with offline support
        const dashboardData = await OfflineAPI.fetchStudentDashboard()
        
        if (dashboardData) {
          const statsData = {
            ...dashboardData.stats,
            weeklyXP: dashboardData.stats?.weeklyXP || 0,
            monthlyXP: dashboardData.stats?.monthlyXP || 0,
            rank: dashboardData.stats?.rank || 0,
            nextLevelXP: dashboardData.stats?.nextLevelXP || 100
          }
          
          const questsData = {
            ...dashboardData.quests,
            streakData: dashboardData.quests?.streakData || { current: 0, best: 0, lastCompleted: "" }
          }
          
          const academicDataProcessed = {
            recentTests: dashboardData.academic?.recentTests || [],
            upcomingExams: dashboardData.academic?.upcomingExams || [],
            subjectPerformance: dashboardData.academic?.subjectPerformance || [],
            overallGPA: dashboardData.academic?.overallGPA || 0,
            loading: false
          }
          ...
          // Cache for sessionStorage as well
          sessionStorage.setItem(`${dashboardCacheKey}_dashboard`, JSON.stringify({
        ...
        // Fetch other data with regular fetch (these are less critical)
        const [schoolRes, announcementsRes, pollsRes] = await Promise.allSettled([
        ...
        if (schoolRes.status === 'fulfilled' && schoolRes.value.ok) {
          ...
          sessionStorage.setItem(`${dashboardCacheKey}_school`, JSON.stringify(schoolData.schoolInfo))
        }
        ...
        if (announcementsRes.status === 'fulfilled' && announcementsRes.value && announcementsRes.value.ok) {
          ...
          sessionStorage.setItem(`${dashboardCacheKey}_announcements`, JSON.stringify(announcementsData.announcements || []))
        }
        ...
        if (pollsRes.status === 'fulfilled' && pollsRes.value.ok) {
          ...
          sessionStorage.setItem(`${dashboardCacheKey}_polls`, JSON.stringify(pollsData.polls || []))
        }
        
        setLoading(false)
        devLog('🏁 [DASHBOARD] All data loaded with PWA offline support!')
        return
      } catch (error) {
        console.error('❌ [DASHBOARD] OfflineAPI error:', error)
        // Fall back to regular fetch if OfflineAPI fails
      }
      ...
      // Process dashboard data (fallback path)
      let dashboardDataToCache: any = null
      if (dashboardRes && dashboardRes.status === 'fulfilled' && dashboardRes.value.ok) {
