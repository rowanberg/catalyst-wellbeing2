...
export default function TeacherStudentsPage() {
  ...
  // Load grades function - moved to component scope
  const loadGrades = async (schoolId?: string) => {
    try {
      ...
      const response = await fetch(`/api/teacher/grades?school_id=${finalSchoolId}`)
      if (response.ok) {
        const data = await response.json()
  ...
  // Load teacher's assigned classes with caching
  useEffect(() => {
    const loadAssignedClasses = async () => {
      ...
      try {
        ...
        const [analyticsResponse, classesResponse] = await Promise.all([
          fetch(`/api/teacher/dashboard-analytics?teacher_id=${user.id}`),
          fetch(`/api/teacher/class-assignments?teacher_id=${user.id}`)
        ])
        ...
        if (analyticsResponse.ok) {
          ...
          if (analyticsData.school_id) {
            schoolId = analyticsData.school_id
          }
        }
        ...
        if (classesResponse.ok) {
          ...
          if (data.success && data.assignments && data.assignments.length > 0) {
            ...
            // Optimized transformation with early filtering
            const transformedClasses = data.assignments
              .filter((assignment: any) => assignment.classes?.id) // Only valid assignments
              .map((assignment: any) => {
        ...
        // Fallback to grades view if no valid classes found
        setCurrentView('grades')
        if (schoolId) {
          await loadGrades(schoolId)
        }
        ...
        setCurrentView('grades')
        if (user?.school_id) {
          await loadGrades(user.school_id)
        }
      } finally {
        setLoading(false)
  ...
  // Load classes when grade is selected
  useEffect(() => {
    if (!selectedGrade || !user?.school_id) return

    const loadClasses = async () => {
      setLoadingClasses(true)
      try {
        const response = await fetch(`/api/teacher/classes?school_id=${user.school_id}&grade_level_id=${selectedGrade}&teacher_id=${user.id}`)
        if (response.ok) {
          const data = await response.json()
          setClasses(data.classes || [])
  ...
  useEffect(() => {
    ...
    const loadStudents = async () => {
      ...
      try {
        const response = await fetch(`/api/teacher/students?school_id=${user.school_id}&class_id=${selectedClass}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const handleClassSelect = useCallback((classId: string) => {
    ...
    // The useEffect will automatically load students when selectedClass changes
  }, [])

  const handleAddMoreClasses = useCallback(() => {
    setCurrentView('grades')
    // Load grades if we have school_id
    if (user?.school_id) {
      loadGrades(user.school_id)
    }
  }, [user?.school_id])
  ...
  const handleDeleteClassAssignment = async (classId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        // Remove the class from the assigned classes list
        ...
        setError(errorData.error || 'Failed to remove class assignment')
      }
    } catch (error) {
      console.error('Error removing class assignment:', error)
      setError('Failed to remove class assignment. Please try again.')
    } finally {
  ...
  // Removed duplicate loadGrades function - using the one with schoolId parameter above
  ...
  const handleAssignTeacherToClass = async (classId: string, isAssigned: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        // Refresh classes to show updated assignment status
        const classesResponse = await fetch(`/api/teacher/classes?school_id=${user.school_id}&grade_level_id=${selectedGrade}&teacher_id=${user.id}`)
        if (classesResponse.ok) {
          const data = await classesResponse.json()
          setClasses(data.classes || [])
        }
        
        // Also refresh assigned classes list
        const assignedResponse = await fetch(`/api/teacher/class-assignments`)
        if (assignedResponse.ok) {
          const assignedData = await assignedResponse.json()
          if (assignedData.assignments && assignedData.assignments.length > 0) {
            const transformedClasses = assignedData.assignments
              .filter((assignment: any) => assignment.classes) // Filter out assignments without class data
              .map((assignment: any) => {
                const classData = assignment.classes
                return {
                  id: classData?.id || assignment.class_id,
                  class_name: classData?.class_name || `Class ${assignment.class_id?.substring(0, 8) || 'Unknown'}`,
                  class_code: classData?.class_code || 'N/A',
                  subject: classData?.subject || assignment.subject || 'General',
                  room_number: classData?.room_number || 'TBD',
                  current_students: classData?.current_students || 0,
                  total_students: classData?.current_students || 0,
                  max_students: classData?.max_students || 30,
                  grade_level: classData?.grade_levels?.grade_level || 'Unknown',
                  grade_name: classData?.grade_levels?.grade_name || `Grade ${classData?.grade_levels?.grade_level || 'Unknown'}`,
                  is_primary_teacher: assignment.is_primary_teacher || false,
                  assigned_at: assignment.assigned_at
                }
              })
        ...
        <AnimatePresence>
          {currentView === 'assigned-classes' && (
            ...
            >
                ...
                <div className="text-center py-12">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      // Retry loading
                      if (user?.id) {
                        // Call the function from the useEffect
              ...
              {!loadingAssignedClasses && !error && assignedClasses.length > 0 ? (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                      ...
                      {assignedClasses.map((cls, index) => (
                        ...
                        >
                          <Card 
                            ...
                            onClick={!showManageMode ? () => handleClassSelect(cls.id) : undefined}
                          >
          ...
          {currentView === 'students' && (
            ...
            >
              <div>
                ...
                {loadingStudents ? (
                <div className="flex items-center justify-center py-12">
                  <ProfessionalLoader size="md" text="Loading students..." />
                </div>
                    ...
                    >
                      <Dialog>
                        <DialogTrigger asChild>
                          <Card className={`group cursor-pointer transition-all duration-500 hover:shadow-2xl hover:scale-[1.02] border-0 bg-gradient-to-br from-white/90 via-white/95 to-blue-50/30 backdrop-blur-lg shadow-xl hover:shadow-blue-200/50 overflow-hidden ${
                            viewMode === 'list' ? 'p-0' : ''
                          }`}>
                            <CardContent className="p-0">
                              ...
                              <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 p-4 relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex items-center justify-between">
                                  <div className="flex items-center gap-4">
                                    {/* Enhanced Avatar */}
                                    <div className="relative">
                                      <div className="w-16 h-16 bg-gradient-to-br from-white/20 to-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg border border-white/20">
                                    ...
                                    <div className="text-white">
                                      ...
                                      <p className="text-blue-100 text-sm opacity-90">{student.email || 'No email'}</p>
                                    </div>
                                  </div>

                                  {/* Mood Indicator */}
                                  <div className="text-right">
                                    <div className="text-2xl mb-1">{moodEmojis[student.current_mood] || '😐'}</div>
                                    <p className="text-xs text-blue-100 capitalize opacity-90">{student.current_mood || 'neutral'}</p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="p-6">
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-4">
                                  <div className="text-center">
                                    <div className="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl flex items-center justify-center shadow-lg">
                                      <Zap className="h-6 w-6 text-white" />
                                    </div>
                                    <div className="font-bold text-lg text-gray-900">{student.xp || 0}</div>
                                    <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">XP</div>
                                  </div>
                                  
                                  <div className="text-center">
                                    <div className="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-xl flex items-center justify-center shadow-lg">
                                      <Star className="h-6 w-6 text-white" />
                                    </div>
                                    <div className="font-bold text-lg text-gray-900">{student.level || 1}</div>
                                    <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">Level</div>
                                  </div>
                                  
                                  <div className="text-center">
                                    <div className="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-green-400 to-emerald-500 rounded-xl flex items-center justify-center shadow-lg">
                                      <TrendingUp className="h-6 w-6 text-white" />
                                    </div>
                                    <div className="font-bold text-lg text-gray-900">{student.streak_days || 0}</div>
                                    <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">Streak</div>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium text-gray-700 capitalize">
                                      {student.wellbeing_status?.replace('_', ' ') || 'Unknown'}
                                    </span>
                                  </div>
                                ...
                                <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                                  <div className="flex items-center gap-2 text-xs text-gray-500">
                                    ...
                                    <span>
                                      {(() => {
                                        // Get class info from either classes or assignedClasses array
                                        const classFromClasses = classes.find(c => c.id === selectedClass)
                                        const classFromAssigned = assignedClasses.find(c => c.id === selectedClass)
                                        const classInfo = classFromClasses || classFromAssigned
                                        
                                        const currentStudents = filteredAndSortedStudents.length
                                        const maxStudents = classInfo?.max_students || classInfo?.maximum_students || 'N/A'
                                        
                                        return `${currentStudents}${maxStudents !== 'N/A' ? `/${maxStudents}` : ''} students`
                                      })()}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="flex-1 h-8 text-xs bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200"
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="flex-1 h-8 text-xs bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200"
                                    onClick={(e) => {
                                      e.stopPropagation()
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-900">{classes.find(c => c.id === selectedClass)?.class_name || 
                                   classes.find(c => c.id === selectedClass)?.name || 
                                   `Class ${classes.find(c => c.id === selectedClass)?.class_code || selectedClass?.substring(0, 8) || 'Unknown'}`}</p>
                                </div>
                                <div>
