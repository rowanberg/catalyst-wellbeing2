...
function TeacherDashboardContentOld({ user, profile }: { user: any, profile: any }) {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative min-h-screen">
      <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-indigo-500/5 to-purple-500/5" />
      <FloatingParticles count={20} className="fixed inset-0 z-0" />
      
      {/* Real-time Notifications */}
      <div className="fixed top-2 right-2 sm:top-4 sm:right-4 z-50 max-w-xs sm:max-w-sm">
      ...
      <div className="max-w-7xl mx-auto p-3 sm:p-6 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 space-y-3 sm:space-y-0">
          <div>
            <motion.h1 
              className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-slate-800 via-blue-800 to-indigo-800 bg-clip-text text-transparent mb-1 sm:mb-2"
            ...
            <motion.p 
              className="text-sm sm:text-base text-slate-600 flex items-center gap-2"
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.2 }}
            >
              <GraduationCap className="h-4 w-4 text-indigo-500" />
              Managing your classroom with excellence
            </motion.p>
          </div>
        ...
        {/* Navigation Tabs */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-11 gap-2 sm:gap-3 mb-6 sm:mb-8">
          {[
            { id: 'overview', label: 'Overview', icon: BarChart3, color: 'from-blue-600 to-indigo-600', bgColor: 'from-blue-50 to-indigo-50' },
            { id: 'roster', label: 'Students', icon: Users, color: 'from-emerald-600 to-teal-600', bgColor: 'from-emerald-50 to-teal-50', isLink: true, href: '/teacher/students' },
            { id: 'analytics', label: 'Analytics', icon: Activity, color: 'from-violet-600 to-purple-600', bgColor: 'from-violet-50 to-purple-50' },
            { id: 'shoutouts', label: 'Shout-outs', icon: Star, color: 'from-amber-500 to-orange-500', bgColor: 'from-amber-50 to-orange-50', shortLabel: 'Shouts' },
            { id: 'activities', label: 'Activities', icon: Play, color: 'from-cyan-600 to-blue-600', bgColor: 'from-cyan-50 to-blue-50', shortLabel: 'Activities' },
            { id: 'communication', label: 'Parent Hub', icon: MessageSquare, color: 'from-sky-600 to-cyan-600', bgColor: 'from-sky-50 to-cyan-50', shortLabel: 'Parents' },
            { id: 'interventions', label: 'Interventions', icon: Lightbulb, color: 'from-orange-600 to-red-500', bgColor: 'from-orange-50 to-red-50', shortLabel: 'Toolkit' },
            { id: 'quests', label: 'Quests', icon: Trophy, color: 'from-rose-500 to-pink-600', bgColor: 'from-rose-50 to-pink-50', shortLabel: 'Quests' },
            { id: 'blackmarks', label: 'Black Marks', icon: AlertTriangle, color: 'from-red-600 to-rose-600', bgColor: 'from-red-50 to-rose-50', shortLabel: 'Marks' },
            { id: 'messaging', label: 'Messages', icon: MessageCircle, color: 'from-indigo-600 to-blue-600', bgColor: 'from-indigo-50 to-blue-50', shortLabel: 'Messages' },
            { id: 'incidents', label: 'Incidents', icon: Shield, color: 'from-slate-600 to-gray-600', bgColor: 'from-slate-50 to-gray-50', shortLabel: 'Incidents' }
          ].map((tab) => {
            const Icon = tab.icon
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className={`relative overflow-hidden rounded-xl p-3 sm:p-5 transition-all duration-500 group ${
                  activeTab === tab.id
                    ? `bg-gradient-to-r ${tab.color} text-white shadow-2xl shadow-${tab.color.split('-')[1]}-500/25`
                    : `bg-gradient-to-br ${tab.bgColor} backdrop-blur-sm text-slate-700 hover:shadow-xl hover:shadow-${tab.color.split('-')[1]}-500/10 border border-white/50`
                }`}
              >
          ...
          <AnimatePresence>
                ...
                {[
                  ...
                  return (
                    <motion.button
                      ...
                      whileHover={{ scale: 1.03, y: -4 }}
                      whileTap={{ scale: 0.97 }}
                      className="p-4 sm:p-6 bg-gradient-to-br from-white/80 to-white/60 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-700 text-left group relative overflow-hidden"
                    >
            ...
            {activeTab === 'shoutouts' && (
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border-0 p-8">
                <div className="mb-6">
                  <h2 className="text-2xl font-bold text-slate-800 mb-2 flex items-center gap-3">
                    <div className="p-2 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl text-white">
                      <Star className="h-6 w-6" />
                    </div>
                    Positive Shout-Outs
                  </h2>
                  <p className="text-slate-600">Recognize and celebrate student achievements</p>
                </div>
                <ShoutOutsSystem />
              </div>
              </motion.div>
            ...
            >
              <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>
                    Secure incident documentation and reporting system
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <p className="text-gray-600">Document and track incidents with secure, timestamped entries.</p>
                    <Button 
                      className="w-full"
