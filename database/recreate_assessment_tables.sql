-- Recreate Assessment Tables Only
-- This script recreates only the assessment-related tables that were dropped
...
-- Recreate student_class_assignments table (since it was dropped)
...
-- Recreate parent_child_relationships table (since it was dropped)
...
DO $$
BEGIN
    RAISE NOTICE 'SUCCESS: Assessment tables recreated successfully!';
    RAISE NOTICE 'Tables: assessments, assessment_grades, grade_templates, offline_grade_sync, student_class_assignments, parent_child_relationships';
    RAISE NOTICE 'Functions: calculate_letter_grade, get_assessment_analytics';
