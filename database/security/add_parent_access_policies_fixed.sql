-- ============================================================================
-- Add Parent Access RLS Policies - FIXED VERSION
...
-- ============================================================================
-- FIXED: post_reactions table uses parent_id column, NOT user_id
...
-- FIXED: Changed from user_id to parent_id to match actual table schema
...
-- ============================================================================
CREATE POLICY "Parents can view their children's wallets" ON public.student_wallets
FOR SELECT TO authenticated
USING (
    student_id IN (
        SELECT pcr.child_id 
...
-- ============================================================================
-- FIXED: parent_notifications table uses parent_id, NOT user_id
...
-- ============================================================================
CREATE POLICY "Parents can view messages with their children" ON public.student_messages
FOR SELECT TO authenticated
USING (
    sender_id IN (
        SELECT p.id FROM public.profiles p WHERE p.user_id = auth.uid()
...
BEGIN
    ...
    RAISE NOTICE '4. post_reactions - View and manage reactions (uses parent_id)';
    ...
    RAISE NOTICE '9. parent_notifications - Manage own notifications (uses parent_id)';
    ...
    RAISE NOTICE '- Profiles table policies omitted to avoid circular RLS';
    RAISE NOTICE '';
